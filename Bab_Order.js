...
function handleOnChange(e) {
    // 선택된 데이터 가져오기
    const data = e.value.replace("3-", "");

    // 데이터 출력
    document.getElementById("BabOrder").innerText = "우리반: "+changeOrder(Number(data), new Date().getDay()) + "번째 (12:" + GoTime[changeOrder(Number(data), new Date().getDay())] + " 출발)";
}
